...
class QuizPageController: UIViewController {
    ...
    // Get total number of questions from the shared QuestionManager
    var totalQuestions: Int {
        return QuestionManager.shared.questions.count
    }
    
    // Create an array of option buttons to iterate over
    ...
    func showQuestion(at index: Int) {
        ...
        // Set options (answers) for buttons using a for loop
        for i in 0..<min(optionButtons.count, question.answers.count) {
            let button = optionButtons[i]
            let answer = question.answers[i]
            button.setTitle(answer.text, for: .normal)  // Set button title to the answer text
        }
    ...
    func resetAnswerButtons() {
        optionButtons.forEach { button in
            ...
            button.isUserInteractionEnabled = true  // Re-enable button interaction
        }
    }
    ...
    // Function that gets called when an option button is selected
    ...
    // Change the button color to green if correct, red if incorrect
    ...
    func showScoreScreen() {
        performSegue(withIdentifier: "showScore", sender: nil)  // Perform segue to ScoreViewController
    }
